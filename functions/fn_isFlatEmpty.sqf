...

{
	...
	if ((abs ((_posNew select 0)-(_position select 0)) < _width/2) && (abs ((_posNew select 1)-(_position select 1)) < _height/2) && (_size > 1) && !(_x in _blacklist) && !(_x isKindOf "men") && !(_x isKindOf "Insect")) exitWith {_r1 = createMarker [format ["A%1", (random 10)], _posX];_r1 setMarkerText format ["A"]; _r1 setMarkerType "hd_objective"; hint format ["%1",_x]; _output = 0;};
