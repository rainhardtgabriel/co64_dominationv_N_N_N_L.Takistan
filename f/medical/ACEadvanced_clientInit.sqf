...
{player addItemToUniform "ACE_elasticBandage"} forEach [1,2,3,4,5,6,7,8,9];
{player addItemToUniform "ACE_packingBandage"} forEach [1,2,3,4,5,6,7,8,9];
...
// Vest
player addItemToVest "TF47_firstAidKititem"; // add this one to Backpack
...
if (_typeOfUnit == "m") then
{
	...
	if (f_param_backpacks >= 1) then {
		...
		(unitBackpack player) addItemCargoGlobal ["TF47_firstAidKititem", 2];
	};
};
