...
Patrols = Patrols;
{
	...
	if (_respawn) then {
		[units (_x select 2) + (_x select 3),3] execVM "Patrols\delayeddeleteVehicles.sqf";
		PatrolMarkers = PatrolMarkers + [_x select 4, _x select 5];
		Patrols = Patrols - [_x];
		[_x select 0, _x select 1] execVM "Patrols\delayedcreatePatrol.sqf";
	};
} forEach Patrols;
//[_typ,_timer,_group, _temp, _start, _end]
